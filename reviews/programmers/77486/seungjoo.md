...
세 개가 다 맞다. 다만 규칙이 바뀌면 세 군데를 모두 고쳐야 하고 하나만 놓쳐도 조용히 틀린다.
**"사람"을 기준으로 한 함수로 통일**하면 분기가 둘로 줄어든다.
...
static void distribute(String person, int money) {
    ...
    if (up < 1) {                      // 10%가 1원 미만 -> 분배 없음, 전액 본인
        result[idx] += money;
        return;
    }
    result[idx] += money - up;         // 90% 본인
    // 추천인이 없으면 up 은 본사 몫이라 그냥 버린다
    if (!parent[idx].equals("-")) distribute(parent[idx], up);
}
...
> **주의 — 두 종료 조건을 묶으면 안 된다.**
> `up < 1` 이면 본인이 **전액**을 갖지만, 추천인이 `"-"` 일 때는 본인이 **90%만** 갖고
> 나머지 10%는 본사로 간다. 지금 코드의 (1)·(3)이 서로 다른 이유가 이것이다.
> 이 리뷰의 초안은 두 조건을 `||` 로 묶었다가 무작위 2만 건 대조에서 19,842건이 어긋나 바로잡았다.
> 위 코드는 같은 방식으로 재검증해 이성일·이승주 구현과 **0건 불일치**를 확인했다.

`solution` 쪽 `if/else` 는 사라지고, `dfs` 의 세 갈래는 두 갈래로 준다.
...
같은 규칙이 세 군데로 갈라졌다. 사람 하나를 기준으로 재귀를 다시 쓰면 세 분기가 둘로 줄고,
